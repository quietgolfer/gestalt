export default function transformer(file, api) {
  ...
  src.find(j.ImportDeclaration).forEach((path) => {
    ...
    const specifier = decl.specifiers.find(node => node.imported.name === 'IconButton' || node.imported.name === 'Button');
    if (!specifier) {
      return;
    }
  ...
  const addAttribute = (attributes, attributeName, propertyValue) => {
    if (propertyValue.value === true) {
      ...
      attributes.unshift(
        j.jsxAttribute(
          j.jsxIdentifier(attributeName), j.jsxExpressionContainer(propertyValue))
      );
    }
  };
