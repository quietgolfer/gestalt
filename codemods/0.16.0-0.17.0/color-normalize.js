export default function transformer(file, api) {
  ...
  src.find(j.ImportDeclaration).forEach((path) => {
    ...
    specifiers = decl.specifiers.filter(node => node.imported.name === 'Icon'
  ...
  return src
    ...
    .forEach((path) => {
      ...
      specifiers.forEach((specifier) => {
        ...
        if (!attrs.some(attr => attr.name.name === 'color'
        || attr.name.name === 'iconColor' || attr.name.name === 'bgColor')) {
          return;
        }
        node.openingElement.attributes = attrs.map((attr) => {
          let attribute;
          if (attr.name.name === 'color' || attr.name.name === 'iconColor' || attr.name.name === 'bgColor') {
            attribute = attr;
          } else {
            return attr;
          }
