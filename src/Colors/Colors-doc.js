...
function Swatch({color, size, shadow}) {
    ...
    return (
        <div className="flex flex-column items-center center">
            <div className={'circle ' + cx(`bg-${color}`)} style={{width: size, height: size, boxShadow: (shadow ? 'inset 0 0 0 2px rgba(0,0,0,0.1)' : 'none')}} />
...
card('Primary Colors',
    ...
    <div className="flex mxn1 justify-center flex-wrap">
        <div className="px1 col-3 mb2">
            {Swatch({color: 'white', size: '6rem', shadow: true})}
        </div>
        <div className="px1 col-3 mb2">
...
card('Extended Palette',
    ...
    <div className="flex content-stretch">
        {alts.map(g =>
            <div className="flex flex-column col-12 justify-center flex-wrap">
                {g.map(color =>
