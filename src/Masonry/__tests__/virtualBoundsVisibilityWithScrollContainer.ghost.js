...
describe('Masonry > virtual bounds visibility /w scroll container', () => {
  it('Calculates correct virtual bounds when masonry is offset', async () => {
    ...
    await ghost.open(`http://localhost:3000/Masonry?scrollContainer=1&offsetTop=${VIRTUALIZED_TOP}`, {
      viewportSize: {
        width: 800,
        height: 800,
      },
    ...
    await ghost.script((scrollToY, selector) => {
      const container = document.querySelector(selector);
