...
describe('Masonry > Item Removal', () => {
  it('Masonry will reflow when removing an item from props.', async () => {
    ghost.close();
    ...
    await ghost.script((proxiedItemData) => {
      window.dispatchEvent(new CustomEvent('set-masonry-items', {
        detail: {
    ...
    for (let i = 0; i < newItems.length; i += 1) {
      ...
      assert.equal(renderedheight, expectedHeight, `item ${i} has a height of ${expectedHeight}`);
    }
  });
});
